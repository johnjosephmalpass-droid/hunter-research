...
**L8 — Prior contradictory evidence.** An earlier retrospective pilot ("v3 Golden," see `V3_GOLDEN_*` constants in `config.py`) produced Stratum D < Stratum B, contradicting the pre-registered primary hypothesis. That pilot disabled web search in the kill phase (`RETROSPECTIVE_DISABLE_WEB_SEARCH = True`), which removes the channel through which cross-silo mechanism verification operates. The summer 2026 study runs with web-searched kill rounds, which is the regime the primary hypothesis is actually about. If the summer study also contradicts H1, the pre-registered decision rule rejects the primary and ships the null paper; the framework then requires structural revision, not recalibration. Users should read the pre-freeze corpus with this prior in mind.
